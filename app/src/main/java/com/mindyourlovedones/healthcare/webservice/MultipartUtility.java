...
public class MultipartUtility {
    ...
    public void addFilePartData(HashMap<String, String> hashMapFields) {
        try {
            for (Map.Entry<String, String> entry : hashMapFields.entrySet()) {
                ...
                if (entry.getKey().equalsIgnoreCase("photo")) {
                    ...
                    File postFile = new File(uri.getPath());
                    if (postFile.isFile())
                        addFilePart(entry.getKey().toString(), postFile);
                    else
                        Log.v("PostFile", " Not created");
